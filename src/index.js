...
function setProfile() {
  profileElement.innerHTML = `
    <h2>${dogProfiles[profileIndex].name}, ${dogProfiles[profileIndex].age}</h2>
    <p>${dogProfiles[profileIndex].bio}</p>
  `
  profileContainer.getElementsByClassName.backgroundImage = `
    url("${dogProfiles[profileIndex].avatar}")
  `
  setReaction()
}
...
function showNextProfile() {
  ...
  if (dogProfiles.length - 1 === profileIndex) {
    profileIndex = -1
  }
  profileIndex++
  setProfile()
